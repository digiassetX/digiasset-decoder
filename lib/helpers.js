...
module.exports.verifyRule=async(assetRules,tx,changes,assetId)=>{
    ...
    if (assetRules.vote!==undefined) {
        ...
        if (assetRules.vote.cutoff!==0n) {
            if (assetRules.vote.cutoff>=1577836800000n) {
                if (BigInt(tx.time)>=assetRules.vote.cutoff/1000n) throw "Sent after expiry.  Asset no longer exists";
            } else {
                if (BigInt(tx.height) > assetRules.vote.cutoff) throw "Sent after expiry.  Asset no longer exists";
            }
        }
    ...
    if (assetRules.expires!==undefined) {
        ...
        if (assetRules.expires>=1577836800000n) {
            if (BigInt(tx.time)>=assetRules.expires/1000n) throw "Sent after expiry.  Asset no longer exists";
        } else {
            if (BigInt(tx.height) > assetRules.expires) throw "Sent after expiry.  Asset no longer exists";
        }
    }
