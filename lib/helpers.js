...
module.exports.verifyRule=async(assetRules,tx,changes,assetId)=>{
    if (assetRules.signers!==undefined) {
        ...
        if (weight<assetRules.signers.required) throw new ExpectedError("Required signer not present");  //User just burned there asset
    }
    if (assetRules.royalties!==undefined) {
        ...
        for (let address in assetRules.royalties) {
            ...
            if (!didSend(tx,address,minAccepted)) throw new ExpectedError("Required royalty not present.");  //User just burned there asset
        }
    }
    if (assetRules.kyc!==undefined) {
        ...
        for (let address in changes[assetId]) {
            ...
            if (kyc===undefined) throw new ExpectedError("Sent to non kyc address");  //User just burned there asset
            if ((kyc.revoked!==undefined)&&(tx.height>=kyc.revoked))  throw new ExpectedError("Sent to non kyc address");  //User just burned there asset
            if (isBan) {
                //if country is on list then illegal
                if (list.indexOf(kyc.country)!==-1) throw new ExpectedError("Sent to not allowed country");  //User just burned there asset
            } else {
    ...
    if (assetRules.vote!==undefined) {
        ...
        if (assetRules.vote.cutoff!==0n) {
            if (assetRules.vote.cutoff>=1577836800000n) {
                if (BigInt(tx.time)>=assetRules.vote.cutoff/1000n) throw new ExpectedError("Sent after expiry.  Asset no longer exists");
            } else {
                if (BigInt(tx.height) > assetRules.vote.cutoff) throw new ExpectedError("Sent after expiry.  Asset no longer exists");
            }
        }
        ...
        if (!assetRules.vote.movable) { //if movable then always a yes so check if not movable
            ...
            for (let address in changes[assetId]) {
                if (changes[assetId][address]>0n) {
                    ...
                    if (allowed[address]===undefined) throw new ExpectedError("Sent to non vote address");  //User just burned there asset
                }
            }
        }
    }
    if (assetRules.expires!==undefined) {
        //see if asset has expired
        if (assetRules.expires>=1577836800000n) {
            if (BigInt(tx.time)>=assetRules.expires/1000n) throw new ExpectedError("Sent after expiry.  Asset no longer exists");
        } else {
            if (BigInt(tx.height) > assetRules.expires) throw new ExpectedError("Sent after expiry.  Asset no longer exists");
        }
    }
    if (assetRules.deflate!==undefined) {
        //count how many assets have been burned
        let burns=0n;
        for (let address in changes[assetId]) if (address!=="") burns-=changes[assetId][address];
        if (burns<assetRules.deflate) throw new ExpectedError("Not enough assets burned for deflate rule");
    }
}