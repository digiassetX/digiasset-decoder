 ...
 * }}inputs
 * @param {int} timeout - max length of time to look for vote data.  default 10min
 * @return {Promise<DigiAssetIssuance|boolean>}
 */
module.exports=async(inputs,timeout=600000)=>{
    ...
    try {
        ...
        if (amountCreated>18014398509481983) throw new ExpectedError("out of range");
        ...
        if ((opcode===3)||(opcode===4)) {
            ...
            do {
                ...
                switch (ruleCode) {
                    ...
                    //vote/expires
                    case 4:
                        if (rules.rewritable===true) throw new ExpectedError("Invalid Rule Detected: Votes can not be part of rewritable rule asset");
                        ...
                        if ((voteLength===0)&&(cutoff!==0n)) {
                            ...
                            let voteOptions = await lookup.getVotes(cid,timeout);
        ...
        if ((locked)&&(amountCreated===0n)) throw new ExpectedError("No assets possible to create");
        ...
        try {
            while (assetCommandData.remaining > 8) {
                ...
                if (amountLeft < 0n) throw new ExpectedError("Not enough assets created");
